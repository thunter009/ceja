...
def nysiis(s: str) -> Optional[str]:
    return None if s == None else J.nysiis(s)


@udf(returnType=StringType())
def metaphone(s: str) -> Optional[str]:
    return None if s == None else J.metaphone(s)


@udf(returnType=StringType())
def match_rating_codex(s: str) -> Optional[str]:
...
@udf(returnType=StringType())
def porter_stem(s: str) -> Optional[str]:
...
def damerau_levenshtein_distance(s1: str, s2: str) -> Optional[int]:
...
def jaro_similarity(s1: str, s2: str) -> Optional[float]:
    return None if s1 == None or s2 == None else J.jaro_similarity(s1, s2)


@udf(returnType=FloatType())
def jaro_winkler_similarity(s1: str, s2: str) -> Optional[float]:
